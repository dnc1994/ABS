...
function BillCtrl($scope, $location, $translate) {
    ...
    $scope.query = function (start_time, end_time) {
        api.bill(start_time, moment(end_time).add(1, 'days').format('YYYY/MM/D'), function (data) {
            ...
            $scope.bill.forEach(function (bill) {
                if (bill.type == 'outgoings') {
                    $scope.chartOutgoing.labels.push((new Date(bill.time = bill.out_time)).toJSON().split('T')[0]);
                    $scope.chartOutgoing.data[0].push(bill.out_total);
                    $scope.chartTotal.labels.push((new Date(bill.time = bill.out_time)).toJSON().split('T')[0]);
                    $scope.chartTotal.data[1].push(bill.out_total);
                    $scope.chartTotal.data[0].push(null);
                }
                else if (bill.type == 'incomes') {
                    $scope.chartIncome.labels.push((new Date(bill.time = bill.in_time)).toJSON().split('T')[0]);
                    $scope.chartIncome.data[0].push(bill.in_total);
                    $scope.chartTotal.labels.push((new Date(bill.time = bill.in_time)).toJSON().split('T')[0]);
